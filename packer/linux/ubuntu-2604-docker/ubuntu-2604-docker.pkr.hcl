...
variable "vm_name" {
  type    = string
  default = "ubuntu-2604-docker-packer"
}
variable "template_name" {
  type    = string
  default = "tmpl-ubuntu-2604-docker"
}
...
variable "docker_disk_size" {
  type        = string
  default     = "300G"
  description = "Groesse der zweiten Disk fuer /docker"
}
...
variable "iso_file" {
  type        = string
  description = "Proxmox Storage-Pfad zur ISO (z.B. datacenter:iso/ubuntu-26.04-beta-live-server-amd64.iso)"
}
...
variable "git_release_tag" {
  type        = string
  default     = "unreleased"
  description = "Semantic Release Version (z.B. ubuntu-2604-docker/v1.0.0)"
}
...
source "proxmox-iso" "ubuntu-docker" {
  ...
  # Docker Disk (/docker)
  disks {
    disk_size    = var.docker_disk_size
    storage_pool = var.vm_storage_pool
    type         = "scsi"
    io_thread    = true
    discard      = true
  ...
  template_description = join("\n", [
    "# Ubuntu 26.04 LTS Docker-Node Template",
    "",
    "| Eigenschaft | Wert |",
    "|---|---|",
    ...
    "| **OS Disk** | ${var.vm_disk_size} |",
    "| **Docker Disk** | ${var.docker_disk_size} (/docker, XFS) |",
  ...
  tags = "os-ubuntu;v-2604;docker;packer;building;build-${formatdate("YYYYMMDD", timestamp())}"
}
...
build {
  ...
  # Ansible Provisioner fuer Base-Hardening + Docker Setup
  provisioner "ansible" {
